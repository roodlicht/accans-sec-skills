...
2. **Secrets-gate** — roep `secrets-scanner` aan, scope = changed files + commit-delta op deze branch. Tooling: `gitleaks detect --source . --log-opts="origin/main..HEAD"` of equivalent. Verified hit = blocker, geen uitzondering.
...
4. **Dep-vuln-gate** — roep `cve-triage` aan op de gewijzigde lockfiles. Blocker wanneer: CVE op CISA KEV, of reachable pre-auth RCE (zie `cve-triage` fase 3). Bij `--full-deps`: check de complete dep-graph, niet alleen wat de diff toevoegt.

5. **Policy-uitzonderingen honoreren.** Twee bronnen, in deze volgorde:
   - Repo-level: `.security-gate.yaml` met `allow:` entries die elk `id` (CVE/rule-id/secret-hash), `reason` en `expires` (YYYY-MM-DD) moeten bevatten. Missend `reason` of verlopen `expires` = uitzondering telt niet.
