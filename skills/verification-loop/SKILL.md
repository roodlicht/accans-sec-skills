...
De skill triggert voor Claude zelf, niet voor de eindgebruiker. Activeer 'm wanneer je op het punt staat non-triviale output af te leveren en nog één pass kunt doen. Concreet:

- Je hebt een analyse, review, rapport, patch, threat-model, runbook of beleidsstuk geschreven en staat op het punt hem aan de gebruiker aan te bieden.
- De gebruiker vraagt expliciet om een second-pass: "check je eigen werk", "zeker weten?", "zijn er gaten?", "red-team dit", "speel advocaat van de duivel".
- Een andere security-skill (bv. `security-review`, `threat-modeler`, `pentest-reporter`, `ir-runbook`, `gdpr-pia`) heeft zojuist output geproduceerd — dan is deze loop de laatste stap voor levering.
...
Werk sequentieel door de stappen. Verzamel bevindingen terwijl je doorloopt; formuleer pas een verdict als alle stappen gedaan zijn. Geen stap overslaan omdat hij "niet van toepassing lijkt" — de stappen die je wil overslaan zijn vaak precies waar de gaten zitten.
...
6. **Security-verdict.** Bovenop het Laag-1-verdict, flag één:
